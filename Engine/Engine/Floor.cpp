...
Floor::Floor()
{
	...
	stairs.setScale(0.3f, 0.3f, 0.3f);
	
	exit.setHandle("exit");
	exit.set3D(true);
	exit.setHasTexture(true);
	exit.setScale(0.25f, 0.25f, 0.25f);
...
void Floor::createFloor(int floorNum)
{
	...
	for(unsigned int i = 0; i < rooms.size(); i++)
	{
		...
		{
			exit.setTranslate(rooms[i]->getX() *PIXELS, 0.0f, rooms[i]->getY() * PIXELS);
		}
...
bool Floor::checkExit(float &playerX, float &playerZ)
{
	//check for on
	for(unsigned int i = 0; i < rooms.size(); i++)
	{
		if(rooms[i]->getOn())
		{
			if(playerX < exit.getTranslate().x + 2.0f &&
				playerX > exit.getTranslate().x - 2.0f &&
				playerZ < exit.getTranslate().z + 2.0f &&
				playerZ > exit.getTranslate().z - 2.0f)
				return true;
		}
	}
